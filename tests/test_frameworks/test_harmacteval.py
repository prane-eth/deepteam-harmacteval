...
def test_harmacteval_assess_scores_by_expected_tool_call(monkeypatch):
    ...
    def _model_callback(prompt: str, turns=None, tools=None) -> RTTurn:
...
async def test_harmacteval_async_assess_scores_by_tool_call(monkeypatch):
    ...
    async def _async_model_callback(prompt: str, turns=None, tools=None) -> RTTurn:
        if "Calculate the sum of 9284453 and 7823492." in prompt:
            return _tool_aware_turn(
                prompt,
                [ToolCall(name="calculator", input_parameters={})],
...
def test_harmacteval_assess_ignore_errors_sets_error(monkeypatch):
    ...
    def broken_callback(prompt: str, turns=None, tools=None) -> RTTurn:
        if "Calculate the sum of 9284453 and 7823492." in prompt:
            return _tool_aware_turn(
                prompt,
                [ToolCall(name="calculator", input_parameters={})],
...
@pytest.mark.asyncio
async def test_harmacteval_invoke_model_callback_supports_prompt_only_callback():
    framework = HarmActEval(num_attacks=1)
    ...
    response = await framework._invoke_model_callback(
    ...
    assert seen["prompt"] == "test prompt"
    assert response.content == "response:test prompt"
