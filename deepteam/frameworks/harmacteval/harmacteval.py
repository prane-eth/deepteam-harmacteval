...
class HarmActEval(AISafetyFramework):
    ...
    def _import_harmacteval_module(self):
        try:
            ...
            target_top_level = self.module_name.split(".", 1)[0]
            if missing_module in {self.module_name, target_top_level}:
                raise ModuleNotFoundError(
                    "HarmActEval integration requires agent-action-guard. "
                    "Install with: pip install 'deepteam[harmacteval]'"
                ) from exc
            # The missing module is a transitive dependency; re-raise to preserve the real cause.
            raise
    ...
    async def a_assess(
        ...
        """
        Asynchronous version of `assess` that keeps progress-bar updates on the
        event loop thread while running the blocking HarmActEval benchmark in a
        background thread.
        """
        try:
            # Run the potentially blocking HarmActEval computation in a worker thread.
            result_map = await asyncio.to_thread(self._run_harmacteval)
            # Apply results to test cases back on the event loop thread.
            self._apply_results_to_test_cases(result_map)
        except Exception:
            if not ignore_errors:
                raise
        ...
        # Update the progress bar from the event loop thread only.
        for _ in self.test_cases:
            update_pbar(progress=progress, pbar_id=task_id)
