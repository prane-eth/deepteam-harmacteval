...
The ADEA threshold is actually 20 employees, not 15 (that's Title VII). And _Henderson v. National Industries_ doesn't exist — the system invented a court case to support its claim. [`Hallucination`](/docs/red-teaming-vulnerabilities-hallucination) specifically targets fabricated facts, citations, statistics, and events — content the model invented rather than distorted.
