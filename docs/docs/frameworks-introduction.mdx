...
DeepTeam supports multiple frameworks — from dataset-based testing (_BeaverTails, Aegis, HarmActionsEval_) to security and governance standards (_MITRE ATLAS, NIST AI RMF, OWASP Top 10 for LLMs_).
...
The HarmActionsEval framework performs action-level safety evaluation.
It is useful for agent safety testing where risk depends on tool/function use behavior, not only generated text.
It evaluates action-level safety using SafeActions@k by checking whether expected harmful tool calls are triggered within the first `k` attempts.
...
[Learn more about HarmActionsEval](/docs/frameworks-harmactionseval)
