...
DeepTeam supports multiple frameworks — from dataset-based testing (_BeaverTails, Aegis, HarmActEval_) to security and governance standards (_MITRE ATLAS, NIST AI RMF, OWASP Top 10 for LLMs_).
...
The HarmActEval framework integrates the Agent Action Guard HarmActEval benchmark to test whether a model executes harmful actions within the first `k` attempts (Harm@k).
It is useful for action-level agent safety testing where the risk is not only model text, but tool and function usage behavior.

- Evaluates harmful tool execution probability using Harm@k
- Maps benchmark outputs into DeepTeam risk assessment format
- Uses the benchmark's OpenAI-based runner and environment configuration
...
[Learn more about HarmActEval](/docs/frameworks-harmacteval)
